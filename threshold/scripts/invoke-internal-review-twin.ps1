...
function Get-ChangedPathsIncludingRenameEndpoints {
    ...
    $nameStatusOutput = @(& git diff --name-status -z -M $Range 2>$null)
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to compute changed path status for $Range."
    }
    $tokens = @(
        foreach ($chunk in $nameStatusOutput) {
            [string]$chunk -split "`0"
        }
    ) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
    for ($i = 0; $i -lt $tokens.Count; $i++) {
        $status = [string]$tokens[$i]
        if ($status -match "`t") {
            $parts = @($status -split "`t" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
            if ($parts.Count -ge 2) {
                $status = [string]$parts[0]
                foreach ($path in @($parts | Select-Object -Skip 1)) {
                    $paths.Add([string]$path)
                }
                if (($status.StartsWith("R") -or $status.StartsWith("C")) -and $parts.Count -eq 2 -and ($i + 1) -lt $tokens.Count) {
                    $paths.Add([string]$tokens[$i + 1])
                    $i++
                }
                continue
            }
...
if ($candidateProvenanceText -notmatch 'insidePreformattedJavadoc') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Preformatted Javadoc content can be auto-wrapped" -AffectedPaths @("threshold/scripts/lib/candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_preformatted_content_not_comment_wrap_candidate") -Description "Javadoc <pre>...</pre> content preserves whitespace and must not enter comment_wrap_cleanup auto-patching."
}
...
if ($candidateTestText -notmatch 'java javadoc lexical state treats line-end pre tag as preformatted opener') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Missing line-end preformatted Javadoc opener hostile fixture" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_line_end_pre_tag_starts_preformatted_content") -Description "The internal test corpus does not exercise a Javadoc <pre tag name at physical line end followed by attributes on the next line."
}
if ($candidateProvenanceText -notmatch 'Update-ThresholdJavadocPreformattedStateInSourceOrder') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Preformatted Javadoc tags are not processed in source order" -AffectedPaths @("threshold/scripts/lib/candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_pre_tag_source_order_preserved") -Description "Javadoc lines containing both </pre> and <pre> must update preformatted state in textual order."
}
if ($candidateTestText -notmatch 'java javadoc lexical state preserves source-order pre tag transitions') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Missing source-order pre tag hostile fixture" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_pre_tag_source_order_preserved") -Description "The internal test corpus does not exercise a Javadoc line that closes one preformatted block and opens another on the same physical line."
}
if ($candidateProvenanceText -notmatch 'Get-ThresholdJavadocPreformattedTransitions') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Inline Javadoc tags can terminate preformatted state" -AffectedPaths @("threshold/scripts/lib/candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_inline_tag_pre_markers_ignored") -Description "Apparent <pre> or </pre> markers inside Javadoc inline tags such as {@code ...} must not change preformatted state."
}
if ($candidateTestText -notmatch 'java javadoc lexical state ignores pre markers inside inline code tags') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Missing inline Javadoc pre marker hostile fixture" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_inline_tag_pre_markers_ignored") -Description "The internal test corpus does not exercise a Javadoc inline tag that contains escaped-looking </pre> text inside a real preformatted block."
}
if ($candidateProvenanceText -notmatch 'InlineTagDepth') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Multiline Javadoc inline tags can terminate preformatted state" -AffectedPaths @("threshold/scripts/lib/candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_multiline_inline_tag_depth_preserved") -Description "Apparent <pre> or </pre> markers must remain ignored while a Javadoc inline tag opened on an earlier line is still active."
}
...
if ($candidateTestText -notmatch 'java javadoc lexical state ignores pre markers inside HTML comments') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Missing HTML-comment Javadoc pre marker hostile fixture" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_html_comment_pre_markers_ignored") -Description "The internal test corpus does not exercise a Javadoc HTML comment that contains escaped-looking </pre> text inside a real preformatted block."
}
if ($candidateTestText -notmatch 'java javadoc lexical state preserves pre state from incoming HTML comment context') {
    $runtimeFindings += New-Finding -ReviewerId "threshold.runtime_literal_reviewer.v0_1" -Category "runtime_literal" -Severity "P2" -Title "Missing incoming HTML-comment-state hostile fixture" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("javadoc_html_comment_rescan_uses_incoming_state") -Description "The internal test corpus does not exercise a multiline HTML comment opened on the same line as <pre> and closed on a later line before a long preformatted candidate line."
}
...
if ($runNextBatchText -notmatch 'Test-ThresholdCommentWrapCandidateLine' -or $runNextBatchText.Contains('foreach ($delimiter in @("/")')) {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Batch split eligibility diverges from discovery" -AffectedPaths @("threshold/scripts/run-next-batch.ps1") -ViolatedPredicates @("batch_comment_wrap_uses_discovery_split_predicate") -Description "Batch comment_wrap_cleanup must use the same conservative split predicate as discovery and must not add URL punctuation delimiters."
}
if ($runNextSliceText -notmatch 'Test-ThresholdCommentWrapCandidateLine') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Slice selection does not revalidate split eligibility" -AffectedPaths @("threshold/scripts/run-next-slice.ps1") -ViolatedPredicates @("slice_comment_wrap_revalidates_split_predicate") -Description "Prepared single-slice candidates must revalidate the conservative split point before selection, not only during mutation."
}
...
if ($candidateTestText -notmatch 'run-next-batch uses shared discovery comment wrap predicate') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Missing batch shared split predicate regression" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("batch_comment_wrap_uses_discovery_split_predicate") -Description "The local reviewer cannot prove batch split eligibility remains aligned with discovery."
}
if ($runNextBatchText -notmatch 'EnsureTrailingNewline' -or $runNextBatchText -notmatch '\$hadTrailingNewline = \$originalText\.EndsWith') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Batch runner can drop the final newline" -AffectedPaths @("threshold/scripts/run-next-batch.ps1") -ViolatedPredicates @("batch_comment_wrap_preserves_final_newline") -Description "comment_wrap_cleanup batch rewriting must preserve the original final newline when the source file already ended with one."
}
if ($candidateTestText -notmatch 'run-next-batch preserves final newline when comment wrap rewrites file') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Missing final-newline preservation regression" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("batch_comment_wrap_preserves_final_newline") -Description "The local reviewer cannot prove batch comment_wrap_cleanup preserves a trailing newline during file rewrite."
}
if ($runNextSliceText -notmatch 'EnsureTrailingNewline' -or $runNextSliceText -notmatch '\$hadTrailingNewline = \$originalText\.EndsWith' -or $runNextSliceText -notmatch '\$lineEnding = Get-LineEnding -Content \$originalText') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Slice runner can rewrite file formatting" -AffectedPaths @("threshold/scripts/run-next-slice.ps1") -ViolatedPredicates @("slice_comment_wrap_preserves_file_formatting") -Description "comment_wrap_cleanup single-slice rewriting must preserve the source file line-ending style and trailing-newline state."
}
if ($candidateTestText -notmatch 'run-next-slice revalidates conservative comment split point before selection') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Missing slice split revalidation regression" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("slice_comment_wrap_revalidates_split_predicate") -Description "The local reviewer cannot prove single-slice selection skips stale candidates with no conservative split point."
}
if ($candidateTestText -notmatch 'run-next-slice write path binds source line ending preservation' -or $candidateTestText -notmatch 'run-next-slice preserves final newline when comment wrap rewrites file') {
    $candidateFindings += New-Finding -ReviewerId "threshold.candidate_class_reviewer.v0_1" -Category "candidate_class" -Severity "P2" -Title "Missing slice file-formatting preservation regression" -AffectedPaths @("threshold/scripts/test-candidate-class-provenance.ps1") -ViolatedPredicates @("slice_comment_wrap_preserves_file_formatting") -Description "The local reviewer cannot prove single-slice comment_wrap_cleanup preserves file line endings and trailing-newline state."
}
...
if ($internalReviewTwinText -notmatch 'Get-ChangedPathsIncludingRenameEndpoints' -or $internalReviewTwinText -notmatch 'git diff --name-status -z -M') {
...
if ($internalReviewTwinText -notmatch '-split "`0"') {
    $evidenceFindings += New-Finding -ReviewerId "threshold.evidence_causality_reviewer.v0_1" -Category "evidence_causality" -Severity "P2" -Title "Review changed paths parse quoted Git output" -AffectedPaths @("threshold/scripts/invoke-internal-review-twin.ps1") -ViolatedPredicates @("review_changed_paths_use_nul_name_status") -Description "Internal review changed-path collection must parse NUL-terminated name-status output so quoted or tab-containing paths cannot bypass forbidden-path matching."
}
...
$results = @(
    ...
    (New-ReviewerResult -ReviewerId "threshold.evidence_causality_reviewer.v0_1" -Findings $evidenceFindings -CoverageClaims @("review_subject_exact_head_bound", "reviewer_inputs_bound_to_resolved_head", "patch_digest_bound_to_base_and_head", "changed_path_digest_bound", "review_patch_bound_to_merge_base", "review_forbidden_path_checks_include_rename_sources", "review_changed_paths_use_nul_name_status"))
)
